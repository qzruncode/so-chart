import type { GuageInstance, SingleGuageInstance } from '@so-chart/types/guage';
...
function GuageChart(this: GuageInstance | SingleGuageInstance) {
...
export default GuageChart as unknown as new () => GuageInstance;

function unBindListener(bindDom: HTMLCanvasElement | SVGElement) {
  select(bindDom).on('.');
}
