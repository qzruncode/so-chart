...
import type { BasePointChartInstance, PointChartInstance } from '@so-chart/types/point';

export function getExposedDataAndDraw(chart: BasePointChartInstance, evtData: MessageData) {
  // 此函数会触发多次
  let exposedDatas: ExposedData[] = [];
  const { chartType, tooltip } = chart;
  if (chartType === 'point') {
    const currentChart = chart as PointChartInstance;
    const data = getPointChartExposedData(currentChart, evtData.position);
...
export function getPointChartExposedData(chart: PointChartInstance, positionsOrXIndex: number[] | number) {
  ...
  showDatasets.forEach(d => {
    const dy = d.data[di]; // 根据索引获取y轴实际数据
