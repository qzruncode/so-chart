import type { PointChartInstance, PointXAxisOptions, PointYAxisOptions } from '@so-chart/types/point';
import { scaleLinear, scaleTime, ticks } from 'd3';

export function computeXAixs(chart: PointChartInstance) {
...
export function computeYAixs(chart: PointChartInstance) {
...
function initXAxis(axis: Required<PointXAxisOptions>, range: readonly [number, number]) {
...
function initYAxis(axis: Required<PointYAxisOptions>, range: readonly [number, number]) {
  let domain, intervalData;
  const { type } = axis;
...
export function drawXAxis(chart: PointChartInstance) {
...
export function drawYAxis(chart: PointChartInstance) {
  ...
  if (context) {
    ...
    if (yAxis.showAixsText === true) {
      ...
      ticks.forEach(d => {
        context.beginPath();
        context.fillText(`${format(d)}${yAxis.unitText}`, x - yAxis.tickSize, yScaleFunc(d));
      });
    }
    context.restore();
  }
...
function computeXTicksData(chart: PointChartInstance) {
  type TextObj = {
    tx: number;
    width: number;
  };
