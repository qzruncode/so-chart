import type { LineDatasetsWithStack, LineOptions } from '@so-chart/types/line';
import { extent } from 'd3';

export default function getStackDatasets(options: LineOptions) {
  const { datasets, yAxis } = options;
  // 将数据做累加
  const showDatasets = datasets.filter(d => !(d.show === false));
  const datasetsWithStack = showDatasets as LineDatasetsWithStack;
